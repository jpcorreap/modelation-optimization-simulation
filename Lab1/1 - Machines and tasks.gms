$ontext
Una compañía posee cuatro máquinas que deben completar cuatro trabajos. Cada máquina debe ser asignada para completar un único trabajo.
El tiempo requerido (en horas) por cada máquina para que complete cada trabajo se muestra en la siguiente figura:
...
Variables
 x(i,j)     Variable binaria que determina si se elige o no el proyecto en la posición i-j.
 v(j)       Variable binaria para garantizar la biyección máquina - trabajos.
 z          Función objetivo que se busca minimizar;
...
res1                     Solo debe haber un valor mínimo por columna
res2                     Solo debe haber un valor mínimo por fila
